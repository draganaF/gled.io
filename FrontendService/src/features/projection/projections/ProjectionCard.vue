<template>
  <div class="card">
    <div class="card-header card-header-rose" data-header-animation="true">
      <img :src="projection.Movie.Picture" height="350px" width="250px" />
    </div>
    <div class="card-body">
      ...
      <div class="card col-md-6">
        ...
        <div class="card-body">
          <h4>
            <b>{{ formatDateTime(projection.Slot) }}</b>
          </h4>
          <h4>
            <b>{{ projection.Movie.Duration }}min</b>
          </h4>
          <h4>
            <b>{{ projection.CinemaHall.Name }}</b>
          </h4>
          <h4>
            <b>{{ projection.Price }} RSD</b>
          </h4>
        </div>
      </div>
    </div>
    <div class="card-footer">
      <Button @click="getTickets" v-if="!!user.role" class="pull-left"
        >Get Tickets</Button
      >
      <Button @click="deleteProjection" v-if="user.role === 'Worker'"
        >Delete</Button
      >
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...mapGetters({
      getScoreMethod: "recensions/getScore",
    }),

    score() {
      return this.getScoreMethod(this.projection.Movie.Id) || 0;
    },
  },
  methods: {
    ...mapActions({
